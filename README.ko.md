...
메뉴바 아이콘은 신호등입니다. 정상일 때 **초록**, 경고 시 **노랑**(맥동), 위험 시 **빨강**(빠른 맥동, 누수 개수 표시).
...
![Daemon Hunter 상태 팝오버 — CRITICAL 상태, AI 분석, 통계(에이전트 42, 메모리 9.2GB, 누수 8), 시스템 지표, 빨간 누수 정리 버튼](img/popover-ko.png)

### 프로세스 상세 창

![프로세스 상세 창 — 상태 점, PID, 경과, 메모리, CPU%, 누수/유휴 원인 컬럼. 정상 sonnet-4-5, 유휴 claude-mem 관찰자, 부모사망(빨강) 관찰자 포함](img/detail-ko.png)
